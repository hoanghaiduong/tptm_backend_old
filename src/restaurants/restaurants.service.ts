...
export class RestaurantsService {
  ...
  async findOneWithAllProduct(id: string): Promise<Restaurant> {
    const restaurant = await this.restaurantRepository.findOne({
      where: { id },
      relations: ['products'],
    });
  ...
  async update(id: string, updateRestaurantDto: UpdateRestaurantDto): Promise<Restaurant> {
    ...
    const mergeData = { ...restaurant, ...updateRestaurantDto };
    const updatedRestaurant = await this.restaurantRepository.save(mergeData);
    return updatedRestaurant;
  }

  async response(id: number, message: string, status: string): Promise<Object> {
